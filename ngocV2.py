        ...
        for (lx,ly,lw,lh) in leftEyes:
            if ( i == True ) : 
                ...
                if ( lx >= fw/2)  and ( lx <= 3*fw/4 ) and ( ly <= fh/3) and (lw*lh/(fw*fh)<=0.09):
                    ...
                    name = "/home/ben/ai/image/eyeAI/dataV3/eyeLeftClose/"+"leftEyesV3_" + str (status) + str (count) + ".jpg" 
        ...
        for (rx,ry,rw,rh) in rightEyes :
            if ( j == True ) :
                if ( rx >= 0 ) and ( rx <=fw/4 )  and ( ry <= fh/3) and (rw*rh/(fw*fh)<=0.09):
                    ...
                    name = "/home/ben/ai/image/eyeAI/dataV3/eyeRightClose/"+"rightEyes_" + str (status) + str (count1) + ".jpg" 
                    #blur_img = cv2.blur(roi_gray1 , ksize=(3, 3))
                    count1 +=1 
                    #cv2.imwrite(name , blur_img2 ) 
